...
md"""
# Frank-Wolfe Algorithms with Explicit Active Set

In this notebook, we will see the difference between standard Frank-Wolfe and some variants based on an active set.

The active set is a set of vertices, such that the current iterate x_t can be represented as a convex combination of these vertices only.
"""
...
begin
	vertices = all_vertices[[1, 3, 4]]
	weights = [0.4, 0.2, 0.4]
	
	# normalization
	weights /= sum(weights)
	sort!(all_vertices, by=get_angle)
end
...
# All plotting functions are courtesy of or inspired by Guillaume Dalle
function plot_distribution!(
	pl, activeset; plot_probadist=true, plot_expectation=true
)
	A = activeset.atoms
