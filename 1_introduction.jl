...
# ╔═╡ b89c1560-7033-11ef-269e-3f8bf3a0e8b3
begin
	using FrankWolfe
	using Plots
...
# Define the negative entropy of a distribution
neg_entropy(x) = sum(xi * log(xi) for xi in x)

# ╔═╡ e064b147-332a-465e-af46-c09e12c63fec
# Alternative syntax
function other_neg_entropy(x)
	return sum(xi * log(xi) for xi in x)
end

# ╔═╡ d9d0ab0d-0550-4aa8-85e5-37ceb6df365c
begin
	plot(0.01:0.01:0.99, neg_entropy, label="negative_entropy");
...
function gradient_neg_entropy(storage, x)
	for i in eachindex(x)
		storage[i] = log(x[i]) + 1
	end
end

# ╔═╡ 34859ff1-c72d-464e-84ab-b4f595a8b902
...
begin
	...
	ffdm  = FiniteDifferences.central_fdm(5, 1)
	grad_finite_differences = FiniteDifferences.grad(ffdm, neg_entropy, xrand)[1]
	gradient_neg_entropy(storage, xrand)
	@show norm(storage - grad_finite_differences)
end
...
begin
	...
	x = range(0.01, 1, length=50)
	y = range(0.01, 1, length=50)
	z = @. contour_func_indicator(x', y)
	contour(x, y, z, levels=10, fill=true)
end

# ╔═╡ c58c9113-cb45-4c82-9137-06f22ee838c9
begin
	lmo = FrankWolfe.ProbabilitySimplexOracle(1.0)
	n = 3
end

# ╔═╡ 80ba763a-e915-41d9-838a-e2218aee8ed9
function safe_neg_entropy(x)
	res = 0.0
	for xi in x
		if xi > 0
			res += xi * log(xi)
	...
	# used to shift the value up to nonnegatives
	return res + log(length(x) + 1)
end

# ╔═╡ 5f5eba4d-5ddf-4b3c-a4ad-36459fed331c
x0 = FrankWolfe.compute_extreme_point(lmo, ones(n))

# ╔═╡ bbaf4a28-e973-4446-a4e0-60a5a6c80069
FrankWolfe.frank_wolfe(safe_neg_entropy, gradient_neg_entropy, lmo, x0, verbose=true)

# ╔═╡ 537215c0-b004-4205-a6e7-6443b46832f7
# What happened?

# ╔═╡ 6f979728-fc6c-46cd-a886-b92967ac9b8f
result = FrankWolfe.frank_wolfe(safe_neg_entropy, gradient_neg_entropy, lmo, [0.1, 0.2, 0.7], verbose=true, trajectory=true)

# ╔═╡ a45a0907-d07c-4855-af5a-fa104eddaca5
# Analyzing the results

# ╔═╡ 85813991-51a3-426d-adf3-ff712446d24d
# Final iterate x_T
result.x

# ╔═╡ bd23daf3-4515-474a-8a3a-d9b8c4fc2065
# Final function value f(x_T)
safe_neg_entropy(result.x), result.primal

# ╔═╡ 27fb0ac1-1d0b-4d62-bc64-eba297dec2df
# Frank-Wolfe gap
begin
	# evaluating the gradient at x_T
	gradient_neg_entropy(storage, result.x)
	v_final = FrankWolfe.compute_extreme_point(lmo, storage)
	final_gap = dot(storage, result.x - result.v)
	(final_gap, result.dual_gap)
end
...
manifest_format = "2.0"
project_hash = "1fbd0d2e5fcf2682962fcc3279db98b58fef442f"
...
git-tree-sha1 = "bc3db0bf76ed9695fa30069b1b66621ccf3515b2"
